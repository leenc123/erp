...
<script>
  import { purchaseReturnOrdersList } from '@/api/purchasing'
  ...
  export default {
    ...
    methods: {
      ...
      list() {
        this.loading = true;
        purchaseReturnOrdersList(this.searchForm).then(data => {
      ...
      detial(item) {
        this.$router.push({ path: '/purchasing/return_record_detail', query: { id: item.id } });
      },
    },
    mounted() {
      this.initialize();
