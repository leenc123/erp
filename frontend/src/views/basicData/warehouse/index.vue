<template>
  <div>
    <a-card title="仓库">
      <a-row :gutter="16">
        ...
        <div style="margin-bottom: 12px; float: right;">
          <a-button type="primary" icon="plus" style="margin: 0 8px;" @click="openFormModal(form)">新增仓库</a-button>
        </div>
      </a-row>
    ...
    <form-modal v-model="visible" :form="targetItem" :usersOptions="usersOptions" @create="create" @update="update" />
...
<script>
  ...
  import { warehouseExport } from '@/api/export'
  import { warehouseTemplate, warehouseImport } from '@/api/import'
  import { warehouseList, warehouseDestroy } from '@/api/basicData'
  import { getWarehouseNumber } from '@/api/data'
  ...
  export default {
    ...
    methods: {
      ...
      list() {
        this.loading = true;
        warehouseList(this.searchForm).then(data => {
      ...
      openFormModal(item) {
        ...
        userOption({ page_size: 999999 }).then(data => {
          this.usersOptions = data.results;
        });
        this.visible = true;
      },
      destroy(id) {
        warehouseDestroy({ id }).then(() => {
      ...
      exportExcel() {
        warehouseExport(this.searchForm).then(resp => {
          exportExcel(resp.data, '仓库字典列表');
        }).catch(err => {
          this.$message.error(err.response.data.error);
        });
      },
      downloadTemplate () {
        warehouseTemplate().then(resp => {
          exportExcel(resp.data, '仓库字典导入模板');
        }).catch(err => {
          this.$message.error(err.response.data.error);
        });
      },
