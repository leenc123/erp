    ...
    <a-card title="收款业务">
      <a-row :gutter="16">
        ...
        <div style="margin-bottom: 12px; float: right;">
          <a-button type="primary" icon="plus" style="margin: 0 8px;" @click="handelAdd(form)">新增收款单</a-button>
        </div>
      </a-row>
      ...
      list() {
        ...
        collectionOrdersList(this.searchForm).then(data => {
          ...
          for(var i=0;i<this.items.length;i++){
            ...
            for(var j=0;j<this.items[i].collection_account_items.length;j++){
              this.items[i].total += parseFloat(this.items[i].collection_account_items[j].collection_amount);
            }
          }
        }).finally(() => {
          this.loading = false;
